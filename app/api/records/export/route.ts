...
export async function GET(req: NextRequest) {
  ...
  if (type === "simplified") {
    rows = records.map((r) => ({
      ...
      "STATUS": r.status ?? "",
      "REASON_FOR_NON_ELIGIBILITY": r.non_eligibility_reason ?? "",
    ...
    rows = records.map((r) => ({
      ...
      "AMENDAREA_VALIDATED": r.amendarea_validated ?? r.amendarea ?? "",
      "AMENDAREA_VALIDATED_CONFIRMED": r.amendarea_validated_confirmed ? "Yes" : "No",
      "ARR_AREA": r.arr_area ?? "",
      "AREA": r.area ?? "",
      "OSAREA": r.osarea ?? "",
      ...
      "CONDONED_AMOUNT": r.condoned_amount ?? r.net_of_reval_no_neg ?? "",
      "CONDONED_AMOUNT_CONFIRMED": r.condoned_amount_confirmed ? "Yes" : "No",
      "FO2": r.fo2 ?? "",
      "FO2_AREA": r.fo2_area ?? "",
