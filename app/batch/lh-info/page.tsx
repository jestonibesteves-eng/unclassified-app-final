...
type PreviewData = {
  ...
  blockedByDates?: { seqno_darro: string; date_encoded: string | null; date_distributed: string | null }[];
};
...
type DoneResult = {
  ...
  notFound: string[];
  outOfJurisdiction: string[];
  updatedRecords: { seqno_darro: string; landowner: string | null; province: string | null; clno: string | null; old_value?: number | null; new_value?: number | null }[];
...
export default function BatchPage() {
  ...
  const [showNumericConfirm, setShowNumericConfirm] = useState(false);
  const [numericConfirmAndConfirm, setNumericConfirmAndConfirm] = useState(false);
  ...
  const blockedByDates = preview?.blockedByDates ?? [];

  function handleConfirmClick(andConfirm = false) {
    if (type === "amendarea" || type === "condoned_amount") {
      setNumericConfirmAndConfirm(andConfirm);
      setShowNumericConfirm(true);
    } else {
  ...
  async function doCommit(andConfirm = false) {
    ...
    setResult({ updated: data.updated, notFound: data.notFound ?? [], outOfJurisdiction: data.outOfJurisdiction ?? [], updatedRecords: data.updatedRecords ?? [], skippedRecords: data.skippedRecords ?? [] });
  ...
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      ...
      if (showNumericConfirm) { setShowNumericConfirm(false); return; }
      if (showRevertConfirm) { setShowRevertConfirm(false); return; }
    }
    document.addEventListener("keydown", handleKeyDown);
    return () => document.removeEventListener("keydown", handleKeyDown);
  }, [showNumericConfirm, showRevertConfirm]);
  ...
  return (
    <div className="max-w-4xl page-enter">
      {/* Header */}
      <div className="mb-6">
        <h2 className="text-[1.6rem] font-bold text-gray-900 leading-tight">LH Info Update</h2>
        ...
        <div className="card-bezel mb-6">
          <div className="card-bezel-inner-open bg-green-50">
            <p className="font-semibold text-green-800 text-sm mb-3">
              ✓ Batch update complete — <strong>{cfg.label}</strong>
            </p>
                  ...
                  <table className="w-full text-[12px]">
                    <thead>
                      <tr className="bg-green-100 text-green-800">
                        ...
                        <th className="text-left px-3 py-2 font-semibold">SEQNO</th>
                        <th className="text-left px-3 py-2 font-semibold">Landowner</th>
                        <th className="text-left px-3 py-2 font-semibold">Province</th>
                        {(type === "amendarea" || type === "condoned_amount") ? (
                          <>
                            <th className="text-right px-3 py-2 font-semibold">Old</th>
                            <th className="text-right px-3 py-2 font-semibold">New</th>
                            <th className="text-right px-3 py-2 font-semibold">Diff</th>
                          </>
                        ) : (
                          <th className="text-left px-3 py-2 font-semibold">CL No.</th>
                        )}
                      </tr>
                    </thead>
                    <tbody>
                      {result.updatedRecords.map((r, i) => {
                        const diff = (type === "amendarea" || type === "condoned_amount") && r.new_value != null
                          ? r.new_value - (r.old_value ?? 0)
                          : null;
                        return (
                          <tr key={r.seqno_darro} className={i % 2 === 0 ? "bg-white" : "bg-green-50/40"}>
                            <td className="px-3 py-1.5 text-gray-400">{i + 1}</td>
                            <td className="px-3 py-1.5 font-mono text-green-800">{r.seqno_darro}</td>
                            <td className="px-3 py-1.5 text-gray-700">{r.landowner ?? "—"}</td>
                            <td className="px-3 py-1.5 text-gray-600">{r.province ?? "—"}</td>
                            {(type === "amendarea" || type === "condoned_amount") ? (
                              <>
                                <td className="px-3 py-1.5 text-right text-gray-500 font-mono">{r.old_value != null ? r.old_value.toLocaleString(undefined, { maximumFractionDigits: 4 }) : "—"}</td>
                                <td className="px-3 py-1.5 text-right text-gray-800 font-mono font-semibold">{r.new_value != null ? r.new_value.toLocaleString(undefined, { maximumFractionDigits: 4 }) : "—"}</td>
                                <td className={`px-3 py-1.5 text-right font-mono font-semibold ${diff == null ? "text-gray-400" : diff > 0 ? "text-blue-600" : diff < 0 ? "text-red-600" : "text-gray-400"}`}>
                                  {diff == null ? "—" : `${diff > 0 ? "+" : ""}${diff.toLocaleString(undefined, { maximumFractionDigits: 4 })}`}
                                </td>
                              </>
                            ) : (
                              <td className="px-3 py-1.5 text-gray-600">{r.clno ?? "—"}</td>
                            )}
                          </tr>
                        );
                      })}
            ...
            {result.skippedRecords.length > 0 && (
              <div className="mb-3">
                <p className="text-[13px] font-semibold text-yellow-700 mb-1">
                  ...
                  <table className="w-full text-[12px]">
                    ...
                    <tbody>
                      {result.skippedRecords.map((r, i) => (
                        <tr key={r.seqno_darro} className={i % 2 === 0 ? "bg-white" : "bg-yellow-50/40"}>
                          <td className="px-3 py-1.5 text-gray-400">{i + 1}</td>
                          <td className="px-3 py-1.5 font-mono text-yellow-800">{r.seqno_darro}</td>
            ...
            <button
              onClick={reset}
              className="mt-4 inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-green-700 hover:bg-green-800 active:scale-[0.97] text-white text-[13px] font-semibold shadow-sm transition-all duration-150"
            >
        ...
        <>
          ...
          <div className="card-bezel mb-4">
            <div className="card-bezel-inner-open">
              ...
              {/* Non-eligibility reason reference */}
              {type === "status" && (
                <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg text-[12px] text-blue-900">
                  <p className="font-semibold mb-2">Recommended REASON values:</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-0.5">
                    {NON_ELIGIBILITY_REASONS.map((r) => (
                      <p key={r} className="font-mono text-blue-700 leading-relaxed">{r}</p>
                    ))}
                  </div>
                  <p className="mt-2 text-[11px] text-blue-500">Any other text is also accepted and will be stored as-is.</p>
                </div>
              )}
          ...
          {preview && (
            <div className="card-bezel mb-4">
              <div className="card-bezel-inner-open">
                ...
                {/* Blocked — has Dates Encoded/Distributed */}
                {type === "status" && blockedByDates.length > 0 && (
                  <div className="mb-3 p-3 bg-red-50 border border-red-300 rounded-lg text-[13px] text-red-800">
                    <p className="font-semibold mb-1">
                      {blockedByDates.length} record{blockedByDates.length !== 1 ? "s" : ""} cannot be set to Not Eligible for Encoding — Dates Encoded/Distributed must be empty:
                    </p>
                    <ul className="space-y-0.5 max-h-32 overflow-y-auto">
                      {blockedByDates.map((b) => (
                        <li key={b.seqno_darro} className="font-mono">
                          {b.seqno_darro}
                          <span className="font-sans text-red-500">
                            {b.date_encoded ? ` — Encoded: ${b.date_encoded}` : ""}
                            {b.date_distributed ? ` — Distributed: ${b.date_distributed}` : ""}
                          </span>
                        </li>
                      ))}
                    </ul>
      ...
      {/* Numeric value confirmation modal (amendarea / condoned_amount) */}
      {showNumericConfirm && preview && typeof window !== "undefined" && createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-6 max-h-[90vh] flex flex-col">
            <h3 className="text-lg font-bold text-gray-900 mb-1">
              Confirm {cfg.label} Update
            </h3>
            <p className="text-sm text-gray-500 mb-4">
              Review the changes below before committing. <strong>{preview.rows.length} record{preview.rows.length !== 1 ? "s" : ""}</strong> will be updated. This action is logged and cannot be undone.
            </p>
            <div className="overflow-auto flex-1 rounded-xl border border-gray-200 mb-5">
              <table className="w-full text-[12px]">
                ...
                <tbody>
                  {preview.rows.map((r, i) => {
                    const oldVal = typeof r.old_value === "number" ? r.old_value : null;
                    const newVal = typeof r.new_value === "number" ? r.new_value : null;
                    const diff = newVal != null ? newVal - (oldVal ?? 0) : null;
                    const fmt = type === "condoned_amount" ? fmt2 : fmt4;
                    return (
                      <tr key={r.seqno_darro} className={i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}>
                        <td className="px-3 py-2 text-gray-400">{i + 1}</td>
                        <td className="px-3 py-2 font-mono text-gray-800 font-semibold">{r.seqno_darro}</td>
                        <td className="px-3 py-2 text-gray-600">{r.landowner ?? "—"}</td>
                        <td className="px-3 py-2 text-right font-mono text-gray-400">{fmt(oldVal)}</td>
                        <td className="px-3 py-2 text-right font-mono text-gray-900 font-semibold">{fmt(newVal)}</td>
                        <td className={`px-3 py-2 text-right font-mono font-semibold ${diff == null ? "text-gray-300" : diff > 0 ? "text-blue-600" : diff < 0 ? "text-red-600" : "text-gray-400"}`}>
            ...
            <div className="flex gap-3 justify-end flex-wrap">
              <button
                onClick={() => setShowNumericConfirm(false)}
                className="px-4 py-2 border border-gray-300 text-gray-600 rounded-lg text-sm hover:bg-gray-50"
              >
                Cancel
              </button>
              <button
                onClick={() => void doCommit(numericConfirmAndConfirm)}
                disabled={loading}
                className="px-5 py-2 rounded-xl text-[13px] font-semibold text-white transition-all active:scale-[0.97]"
                style={numericConfirmAndConfirm
                  ? { background: "linear-gradient(135deg, #f97316 0%, #ea580c 100%)", boxShadow: "0 4px 14px rgba(249,115,22,0.35)" }
                  : { background: "linear-gradient(135deg, #16a34a 0%, #15803d 100%)", boxShadow: "0 4px 14px rgba(22,163,74,0.35)" }
                }
              >
                {loading ? "Updating…" : numericConfirmAndConfirm ? <>Update and Confirm <span className="ml-1">✓✓</span></> : <>Update {preview.rows.length} Record{preview.rows.length !== 1 ? "s" : ""} <span className="ml-1">✓</span></>}
              </button>
            </div>
          </div>
        </div>,
