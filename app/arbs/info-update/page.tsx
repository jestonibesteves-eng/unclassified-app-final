              ...
              <div className="card-bezel-inner-open">
                ...
                {lockedRows.length > 0 && (
                  <div className="mb-3 p-3 bg-amber-50 border border-amber-200 rounded-lg text-[13px] text-amber-800">
                    <strong>{lockedRows.length} record{lockedRows.length !== 1 ? "s" : ""} locked</strong>
                    {cfg.locked
                      ? <> — LH status is at or beyond &quot;For Encoding&quot;, field cannot be updated:</>
                      : <> — cannot be updated (see reason per record):</>
                    }
                    <ul className="mt-1 space-y-0.5 max-h-24 overflow-y-auto">
                      {lockedRows.map((r) => (
                        <li key={`${r.seqno_darro}|${r.arb_id}`} className="font-mono">
                          {r.seqno_darro} / {r.arb_id}
                          <span className="font-sans text-amber-600 ml-1">— {r.lockedReason ?? r.lh_status}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                    ...
                    <table className="w-full text-[13px]">
                      <thead className="bg-green-900 text-white text-[11px] uppercase tracking-wide">
                        <tr>
                          ...
                          {(cfg.locked || lockedRows.length > 0) && <th className="px-3 py-2.5 text-left font-semibold whitespace-nowrap">LH Status</th>}
                      ...
                      <tbody>
                        {preview.rows.map((r, i) => (
                          <tr key={`${r.seqno_darro}|${r.arb_id}`}
                            ...
                            {(cfg.locked || lockedRows.length > 0) && (
